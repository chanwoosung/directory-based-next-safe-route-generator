...
async function resolveRootDir(base: string, type: ProjectType): Promise<string> {
  ...
  throw new Error(`❌ Could not find route root for ${type}`);
}

/**
 * ✅ 폴더 및 파일에서 라우트 경로를 추출 (트리 없이 flat)
 */
async function collectRoutes(
  ...
  // ✅ 이 폴더에 page.tsx / index.tsx 존재 → route로 등록
  ...
  for (const entry of dirs) {
    ...
    // 🧩 parallel route (@segment) → 무시
    if (name.startsWith("@")) continue;

    // 🧩 intercept route ( (..segment) ) → path에 영향 없음, 내부 탐색
    if (/^\(\.\..*\)$/.test(name)) {
    ...
    // 🧩 group route ( (group) ) → path에는 포함하지 않지만 내부 탐색
    if (/^\(.*\)$/.test(name)) {
      const subPath = path.join(dir, name);
      result.push(...(await collectRoutes(subPath, prefix)));
      continue;
    ...
    // ✅ 일반 폴더
    const safeSegment = name.replace(/-/g, "_"); // 하이픈 → 언더바
...
function makeTypeFile(routes: { path: string; params: string[] }[]): string {
  ...
  const union = routes.map((r) => makeInterfaceName(r.path)).join(" | ") || "never";

  return `// Auto-generated by safe-router-generator
...
export async function generateTypeFile(options: GenerateOptions) {
  ...
  async function generate() {
    ...
    console.log(`✅ Routes generated: ${out}`);
  }

  await generate();

  if (watch) {
    console.log(`👀 Watching for route changes in ${routeRoot}`);
